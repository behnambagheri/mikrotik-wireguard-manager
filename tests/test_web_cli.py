...
from wg_users_web import web_cli
...
class TestWebCli(unittest.TestCase):
    ...
    def test_main_sets_env_and_runs(self):
        ...
        self.assertEqual(os.environ["WG_WEB_ENV_FILE"], "x.env")
        self.assertEqual(os.environ["WG_WEB_STATE_FILE"], "x.json")
        fake.run.assert_called_once()
        kwargs = fake.run.call_args.kwargs
        self.assertIn("log_config", kwargs)
        self.assertIn("wg_users_web", kwargs["log_config"]["loggers"])
        self.assertEqual(kwargs["log_config"]["loggers"]["wg_users_web"]["level"], "INFO")

    def test_profile_switch_endpoint_returns_selected_profile(self):
        try:
            from fastapi.testclient import TestClient
            from wg_users_web import web
        ...
        with mock.patch("wg_users_web.web.WebManager", return_value=manager):
            app = web.create_app()
    ...
    def test_root_main_exposes_fastapi_app(self):
        ...
        fake_pkg = types.ModuleType("wg_users_web")
        with mock.patch.dict(sys.modules, {"wg_users_web": fake_pkg, "wg_users_web.web": fake_web}):
