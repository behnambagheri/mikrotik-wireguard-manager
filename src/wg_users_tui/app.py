...
class App:
    ...
    def draw_dashboard(self) -> None:
        ...
        if isinstance(self.client, ApiSslClient):
            mode = "API-SSL" if self.client.use_ssl else "API"
        else:
            mode = "REST"
        self.put(2, 0, f"Router: {self.profile_name}@{self.host}  Mode: {mode}  Poll: {self.last_poll_latency_ms:.0f}ms ({poll_ago} ago)", self.c_header)
